...
class MobilityTrace(object):
    ...
    def __init__ (self, timestamp, cellid, latitude, longitude, eventType=None,
...
#################################################
    ...
    def __repr__ (self):
	    return "'{0}' '{1}' '{2}' '{3}' ".format(self.timestamp,self.cellid,self.latitude,self.longitude)
    ...
    def time_distance2(self, mobilityTrace):
        """ Computes de difference between the timestamp of the eventement passed in arguments and the timestamp of the object that calls the method in seconds"""
        deltaTime = mobilityTrace.timestamp - self.timestamp
...
#################################################
    @classmethod
    def repeated(cls,trialMobilityTraces):
	#new_list = (trialMobilityTraces)
	size=len(trialMobilityTraces)
	new_list=list()
	mobility=list()
	for i in range(size):
	    mt1=trialMobilityTraces[i]
	    #mobility.append(mt1.eventType)
	#print(mobility)
	set_mobility=set(mobility)
	for j in set_mobility:
	    index=mobility.index(j)
	    mt1=trialMobilityTraces[index]
	    new_list.append(mt1)
	#print(set_mobility)
	return new_list
    ...
    @classmethod
    def compute_cumulated_distance_day(cls, trailMobilityTraces):
        """
            This method returns a float representing the cumulated distance of a trail of
            mobility trace per day
            @param: The trail of mobility traces
            @result: the acumulated distance per day
        """
        distance = 0
	h=list()
        size = len(trailMobilityTraces)
        if (size > 1):
            i = 0
            while (i < size-1):
		if(trailMobilityTraces[i].time_distance2(trailMobilityTraces[i+1])==0):
		    distance += float( trailMobilityTraces[i].distance(trailMobilityTraces[i+1]))
		    h.append(distance)
		else:
		    pass
    ...
    @classmethod
    def deviation(cls,trailMobilityTraces,distance,pts=list()):
	size=len(trailMobilityTraces)-1
	v=0
	if distance!=0:
